...
description: >-
  ...
  DO NOT TRIGGER: bdpan-only /apps/bdpan memory-backup (use baidu-drive); local upload/download
  via OpenAPI (use baidu-drive or web UI); non-pan tasks; 文库 PPT (baidu-wenku-aippt).
...
argument-hint: "[check-install|list|search|move|mkdir|push …]"
---
...
| `tasks/` | Task-pack JSON workspace |
...
|---|---|
| PASS | Continue |
| FAIL bridge | `python "$TOOLS\bridge.py"` then re-check |
...
Install copy-paste for users: [INSTALL.md](./INSTALL.md).

### User-facing install blurb

```text
请完成 baidu-pan-connector 环境：
1) python %USERPROFILE%\.codex\skills\baidu-pan-connector\tools\bridge.py
2) Chrome 加载已解压扩展：%USERPROFILE%\.codex\skills\baidu-pan-connector\extension
3) 打开并登录 https://pan.baidu.com ，保持页签
4) python %USERPROFILE%\.codex\skills\baidu-pan-connector\scripts\check_install.py
至 PASS 后再操作。
```
...
| Action | How |
|---|---|
| Health | `python "$TOOLS\pan_task.py" health` |
| List | `python "$TOOLS\pan_query.py" list "/path" --max 200` |
| Exists | `python "$TOOLS\pan_query.py" exists "/path"` |
| Search | `python "$TOOLS\pan_query.py" search "key" --dir "/" --max 50` |
| Writes | Task pack in `$S\tasks\` → `pan_task.py push … --auto --wait` |
...
```powershell
# one-shot
python "$TOOLS\pan_query.py" upload "D:\path\file.pdf" --dest "/remote/dir"
python "$TOOLS\pan_query.py" upload "D:\path\file.pdf" --path "/remote/dir/file.pdf" --ondup overwrite

# task pack
# { "op":"upload", "local":"D:\\path\\file.pdf", "dest":"/remote/dir", "newname":"file.pdf", "ondup":"fail" }
python "$TOOLS\pan_task.py" push "$S\tasks\my-upload.json" --auto --wait
```
...
```powershell
python "$TOOLS\pan_task.py" push "$S\tasks\my.json" --auto --wait
python "$TOOLS\pan_task.py" push "$S\tasks\my.json" --auto --mode safe --wait
```
...
| Doc | When |
|---|---|
| [INSTALL.md](./INSTALL.md) | Setup / Chrome load path |
